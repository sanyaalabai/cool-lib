...
- **generate/header.js**  
	Automatically creates header from configured `header.json` that is in adjacent directory or by path defined in variable `coollibDir`.
Works best if it's placed right after `<body>`. Also add with `type="module"`.
- **generate/footer.js**  
	Automatically creates footer from configured `footer.json` that is in adjacent directory or by path defined in variable `coollibDir`.
Works best if it's placed right before `</body>`. Also add with `type="module"`.
- **generate/sidebar.js**  
